...
def create_student(username, password, first_name, email, phone, hostel, is_admin=False):
    user, created = User.objects.get_or_create(
        username=username,
        defaults={'first_name': first_name, 'email': email, 'is_staff': is_admin, 'is_superuser': is_admin}
    )
    if created:
...
admin = create_student("admin", "admin123", "Admin", "admin@simplymkt.com", "256770000000", "main_campus", is_admin=True)
buyer = create_student("buyer", "studentpass", "Okello", "okello@gulu.ac.ug", "256770000001", "laroo", is_admin=False)
seller = create_student("seller", "studentpass", "Akena", "akena@gulu.ac.ug", "256770000002", "main_campus", is_admin=False)
...
if created4:
    ...
    BundleItem.objects.get_or_create(product=p4, name="Double Wooden Bed & Mattress", estimated_price=250000)
    BundleItem.objects.get_or_create(product=p4, name="Plastic Basin & Bucket Set", estimated_price=15000)
    BundleItem.objects.get_or_create(product=p4, name="Cooking Utensils & Plates", estimated_price=40000)
    BundleItem.objects.get_or_create(product=p4, name="Sony Subwoofer System", estimated_price=120000)
